...
/**
  ...
  */
final case class ObjectSchema(properties: Map[String, JsonSchema] =Map.empty)(implicit val schemaContext: SchemaContext) extends JsonSchema with SchemaWithTransform[ObjectSchema] {

  def toJson = ("type" -> "object") ~ ("properties" -> properties.toList
    .sortBy {
      case (key, _) => key.hashCode
    }.map {
